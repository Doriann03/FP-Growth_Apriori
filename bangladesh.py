# =========================================================
# BANGLADESH - FINAL REPORT VERSION (RAM + CLEAN CSV)
# =========================================================
import pandas as pd
import time
import os
...
def run_and_save(algo_func, matrix, support, name, top_n=20):
    ...
    if success and count > 0:
        try:
            ...
            filename = f"rezultate_{name.lower()}_bangladesh.csv"
            df_export.to_csv(filename, index=False)
            print(f"CSV Salvat: {filename}")
            
        except Exception as e:
            print(f"Eroare generare reguli: {e}")

    return success, duration, peak_mb, count

def main():
    print("\n" + "="*80)
    print("🇧🇩  BANGLADESH - GENERARE RAPORT FINAL")
    print("="*80)

    if not os.path.exists('bangladesh.csv'): print("Lipsă bangladesh.csv"); return
    
    print("Citire date...")
    try: df = pd.read_csv('bangladesh.csv', encoding='latin-1', low_memory=False)
    except: df = pd.read_csv('bangladesh.csv', encoding='cp1252', low_memory=False)
